...
- **`pipeline/`** — prompt assembly. `PromptBuilder` executes an ordered, replaceable stage list (`PromptStages.ts`: hidden-filter → macro ctx → WI scan → prompt slots → history splices → collection → cache depth → render), orchestrating `PromptManager` (prompt slots/order) + a renderer (`ChatCompletionRenderer` or `TextCompletionRenderer`) + `MacroResolver`, `WorldInfoInjector`, `ExampleBuilder`, `RegexEngine`. Same `PromptCollection` feeds chat- and text-completion backends (no separate story-string system).
