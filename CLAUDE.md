...
- **`tts/`** — TTS adapter factory + providers (OpenAI, ElevenLabs, Azure, FishAudio S2, Kokoro FastAPI, GPT-SoVITS, Silero, VITS Simple API, AllTalk, MiniMax, VolcEngine, Qwen/DashScope incl. voice design), served via `/api/tts`.
- **`api/`** — HTTP routers (attachments, characters, chats, files, data maid, models, personas, secrets, stats) for uploads/exports and the non-WS surface, plus the anthropic-like `proxy` router (`/v1/models`, `/v1/messages`, non-streaming — backend configs exposed as models with IDs `${uuid}-${name}`, gated on the `proxyApi.enabled` setting, authenticates with its own `proxyApi.apiKey` rather than the login token); any handler that mutates shared state must call `bus.broadcast()`.
