...
- **`pipeline/`** — prompt assembly. `PromptBuilder` orchestrates `PromptManager` (prompt slots/order) + a renderer (`ChatCompletionRenderer` or `TextCompletionRenderer`) + `MacroResolver`, `WorldInfoInjector`, `ExampleBuilder`, `RegexEngine`. Same `PromptCollection` feeds chat- and text-completion backends (no separate story-string system).
...
- **`services/`** — `GenerationService` (generation lifecycle + tool-call loop, `maxToolRounds` default 100), `ToolRegistry` + `ToolTemplate`, `GroupChatService`, `RAGService` (vectra vector store), `MemoryService`, `SecretService` (encrypted), `RegexEngine`, `DataMaid`.
