...
- **`api/`** — HTTP routers (attachments, characters, chats, files, data maid, models, personas, secrets, stats) for uploads/exports and the non-WS surface, plus the anthropic-like `proxy` router (`/v1/models`, `/v1/messages`, non-streaming — backend configs exposed as models with IDs `${uuid}-${name}`, gated on the `proxy.enabled` setting); any handler that mutates shared state must call `bus.broadcast()`.
