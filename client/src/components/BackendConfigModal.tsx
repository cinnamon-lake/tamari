...
import {
  ...
  formatLogitBias,
  splitStopStrings,
  buildProviderParams as buildProviderParamsPatch,
  seedAdvancedParams,
  fetchModelList,
  extractOpenRouterProviders,
  filterModelsByProvider,
  type ModelInfo,
} from '../lib/backendConfig.js';
...
export function BackendConfigModal(props: { onClose: () => void }) {
  ...
  const fetchModels = async () => {
    ...
    try {
      const items = await fetchModelList();
  ...
  const openrouterProviders = () => extractOpenRouterProviders(availableModels());

  const filteredModels = () => filterModelsByProvider(availableModels(), openrouterProvider());
  ...
  const loadConfigData = (config: NonNullable<typeof state.activeBackendConfig>) => {
    ...
    const seeded = seedAdvancedParams(config.providerParams, profile);
    setAdvancedParams(seeded.values);
    setSamplerDisabled(seeded.disabled);
    setFormLoaded(true);
  };
  ...
  const buildProviderParams = (existing: Record<string, unknown> | undefined): Record<string, unknown> =>
    buildProviderParamsPatch({
      ...
      customBackendId: customBackendId(),
  ...
  const saveConfig = () => {
    ...
    bus.send({
      ...
      patch: {
        ...
        stopStrings: splitStopStrings(stopStrings()),
  ...
  return (
    <Modal
      title={
        <>
          {t('backendConfig.title')} {saving() && <span class="text-sm text-muted">{t('backendConfig.saving')}</span>}
        </>
      }
      onClose={close}
      class="modal settings-modal"
      ariaLabel={t('backendConfig.ariaLabel')}
      data-testid="backend-config-modal"
      data-form-loaded={formLoaded() ? 'true' : 'false'}
    >
      {/* Config Selector */}
      <section class="settings-section">
            ...
            <For each={state.backendConfigs}>
              {(config) => (
                <option
                  ...
                  id={config.id}
                  value={config.id}
                  selected={config.id === activeBackendConfigId()}
                >
                  {config.name}
                </option>
              )}
        ...
        <Show when={activeBackendConfigId()}>{(id) => <IdBadge id={id()} />}</Show>
      ...
      <section class="settings-section">
        ...
        <label class="field-label">
          ...
          >
            <option class="select-option" value="chat">
              {t('backendConfig.generationModeChat')}
            </option>
            <option class="select-option" value="text">
              {t('backendConfig.generationModeText')}
            </option>
          </select>
        </label>
        <label class="field-label">
          ...
          <select
            class="select"
            data-testid="backend-config-provider"
          ...
          >
            <For each={PROVIDERS_BY_MODE[generationMode()]}>
              {(p) => (
                <option class="select-option" id={p.value} value={p.value}>
                  {p.label}
                </option>
              )}
            </For>
        ...
        <Show when={backendProvider() === 'custom'}>
          <label class="field-label">
            ...
            >
              <option class="select-option" value="">
                {t('customBackends.selectBackend')}
              </option>
              <For each={state.customBackends}>
                {(b) => (
                  <option class="select-option" id={b.id} value={b.id}>
                    {b.name}
                  </option>
                )}
              </For>
          ...
          <label class="field-label">
            ...
            >
              <option class="select-option" value="">
                {t('customBackends.delegateDefault')}
              </option>
              <For each={state.backendConfigs.filter((c) => c.id !== activeBackendConfigId())}>
                {(c) => (
                  <option class="select-option" id={c.id} value={c.id}>
                    {c.name}
                  </option>
                )}
              </For>
        ...
        <Show when={backendProvider() === 'openrouter' && openrouterProviders().length > 0}>
          <label class="field-label">
            ...
            >
              <option class="select-option" value="">
                {t('backendConfig.allProviders')}
              </option>
              <For each={openrouterProviders()}>
                {(p, index) => (
                  <option class="select-option" id={`provider-${index()}`} value={p}>
                    {p}
                  </option>
                )}
              </For>
            ...
            fallback={
              <div class="model-picker-row">
                <input
                  ...
                  placeholder={
                    modelsFailed()
                      ? t('backendConfig.modelsFailedPlaceholder')
                      : t('backendConfig.modelNamePlaceholder')
                  }
                />
                <button
                  type="button"
                  ...
                  onClick={fetchModels}
                  title={t('backendConfig.refreshModels')}
                  aria-label={t('backendConfig.refreshModels')}
                ...
                <For each={filteredModels()}>
                  {(m) => (
                    <option class="select-option" id={m.id} value={m.id}>
                      {m.name}
                      {m.contextLength
                        ? ` ${t('backendConfig.modelContextBadge', { n: m.contextLength.toLocaleString() })}`
                        : ''}
                    </option>
                  )}
                </For>
        ...
        <Show when={generationMode() === 'text'}>
          <label class="field-label">
            ...
            >
              <option class="select-option" value="">
                {t('backendConfig.instructNone')}
              </option>
              <option class="select-option" value="alpaca">
                Alpaca
              </option>
              <option class="select-option" value="chatml">
                ChatML
              </option>
              <option class="select-option" value="deepseek-v4-pro">
                DeepSeek V4 Pro
              </option>
              <option class="select-option" value="deepseek-v4-pro-thinking">
                DeepSeek V4 Pro (Thinking)
              </option>
              <option class="select-option" value="gemma4">
                Gemma 4
              </option>
              <option class="select-option" value="gemma4-thinking">
                Gemma 4 (Thinking)
              </option>
              <option class="select-option" value="glm-5.1">
                GLM 5.1
              </option>
              <option class="select-option" value="glm-5.1-thinking">
                GLM 5.1 (Thinking)
              </option>
              <option class="select-option" value="granite-4.0">
                IBM Granite 4.0 / 4.1
              </option>
              <option class="select-option" value="kimi-k2.6">
                Kimi K2.6
              </option>
              <option class="select-option" value="kimi-k2.6-thinking">
                Kimi K2.6 (Thinking)
              </option>
              <option class="select-option" value="kimi-k3">
                Kimi K3
              </option>
              <option class="select-option" value="kimi-k3-thinking">
                Kimi K3 (Thinking)
              </option>
              <option class="select-option" value="llama2">
                Llama 2
              </option>
              <option class="select-option" value="llama3">
                Llama 3
              </option>
              <option class="select-option" value="llama4">
                Llama 4
              </option>
              <option class="select-option" value="minimax-text-01">
                MiniMax Text-01
              </option>
              <option class="select-option" value="mistral">
                Mistral
              </option>
              <option class="select-option" value="mistral-large-2411">
                Mistral Large 2411
              </option>
              <option class="select-option" value="mistral-nemo">
                Mistral Nemo
              </option>
              <option class="select-option" value="mistral-v0.1">
                Mistral v0.1
              </option>
              <option class="select-option" value="mistral-v0.3">
                Mistral v0.3
              </option>
              <option class="select-option" value="mistral-v3">
                Mistral v3
              </option>
              <option class="select-option" value="mistral-v3-thinking">
                Mistral v3 (Thinking)
              </option>
              <option class="select-option" value="nemotron-3">
                NVIDIA Nemotron 3
              </option>
              <option class="select-option" value="nemotron-3-thinking">
                NVIDIA Nemotron 3 (Thinking)
              </option>
              <option class="select-option" value="phi-4-mini">
                Phi-4 Mini
              </option>
              <option class="select-option" value="phi-4-reasoning-plus">
                Phi-4 Reasoning Plus
              </option>
              <option class="select-option" value="qwen3">
                Qwen 3
              </option>
              <option class="select-option" value="qwen3-thinking">
                Qwen 3 (Thinking)
              </option>
              <option class="select-option" value="qwen3.5">
                Qwen 3.5 / 3.6
              </option>
              <option class="select-option" value="qwen3.5-thinking">
                Qwen 3.5 / 3.6 (Thinking)
              </option>
              <For each={customTemplates()}>
                {(tpl) => (
                  <option class="select-option" id={tpl.id} value={tpl.id}>
                    {tpl.name}
                  </option>
                )}
              </For>
        ...
        <div class="sampler-field">
          <label class="field-label" for="sampler-temperature">
            {t('backendConfig.temperature')}
          </label>
        ...
        <div class="sampler-field">
          <label class="field-label" for="sampler-topP">
            {t('backendConfig.topP')}
          </label>
          <div class="sampler-input-group">
            <label class="sampler-input-addon" title={t('backendConfig.samplerEnabled')}>
              <input
        ...
        <div class="sampler-field">
          <label class="field-label" for="sampler-topK">
            {t('backendConfig.topK')}
          </label>
          <div class="sampler-input-group">
            <label class="sampler-input-addon" title={t('backendConfig.samplerEnabled')}>
              <input
        ...
        <div class="sampler-field">
          <label class="field-label" for="sampler-minP">
            {t('backendConfig.minP')}
          </label>
          <div class="sampler-input-group">
            <label class="sampler-input-addon" title={t('backendConfig.samplerEnabled')}>
              <input
        ...
        <div class="sampler-field">
          <label class="field-label" for="sampler-topA">
            {t('backendConfig.topA')}
          </label>
          <div class="sampler-input-group">
            <label class="sampler-input-addon" title={t('backendConfig.samplerEnabled')}>
              <input
        ...
        <div class="sampler-field">
          <label class="field-label" for="sampler-repetitionPenalty">
            {t('backendConfig.repetitionPenalty')}
          </label>
          <div class="sampler-input-group">
            <label class="sampler-input-addon" title={t('backendConfig.samplerEnabled')}>
              <input
            ...
            <input
              ...
              onInput={(e) =>
                markDirty(setRepetitionPenalty)(e.currentTarget.value ? Number(e.currentTarget.value) : null)
              }
            />
          </div>
        </div>
        <div class="sampler-field">
          <label class="field-label" for="sampler-frequencyPenalty">
            {t('backendConfig.frequencyPenalty')}
          </label>
          <div class="sampler-input-group">
            <label class="sampler-input-addon" title={t('backendConfig.samplerEnabled')}>
              <input
            ...
            <input
              ...
              onInput={(e) =>
                markDirty(setFrequencyPenalty)(e.currentTarget.value ? Number(e.currentTarget.value) : null)
              }
            />
          </div>
        </div>
        <div class="sampler-field">
          <label class="field-label" for="sampler-presencePenalty">
            {t('backendConfig.presencePenalty')}
          </label>
          <div class="sampler-input-group">
            <label class="sampler-input-addon" title={t('backendConfig.samplerEnabled')}>
              <input
            ...
            <input
              ...
              onInput={(e) =>
                markDirty(setPresencePenalty)(e.currentTarget.value ? Number(e.currentTarget.value) : null)
              }
            />
          </div>
        </div>
                ...
                {(knob, i) => (
                  <>
                    <Show when={i() === 0 || activeProfile()[i() - 1]?.group !== knob.group}>
                      <h4 class="text-sm text-muted mb-0 mt-sm">{td('backendConfig.adv.group.' + knob.group)}</h4>
                    </Show>
          ...
          <div class="mt-md flex-col-sm">
            ...
            <label class="field-label">
              ...
              <select
                ...
                onChange={(e) =>
                  setOpenrouterReasoningEffort(e.currentTarget.value as AppSettings['openrouter.reasoningEffort'])
                }
              >
                <option class="select-option" value="">
                  {t('backendConfig.optionDefault')}
                </option>
                <option class="select-option" value="xhigh">
                  {t('backendConfig.effortExtremeHigh')}
                </option>
                <option class="select-option" value="high">
                  {t('backendConfig.effortHigh')}
                </option>
                <option class="select-option" value="medium">
                  {t('backendConfig.effortMedium')}
                </option>
                <option class="select-option" value="low">
                  {t('backendConfig.effortLow')}
                </option>
                <option class="select-option" value="minimal">
                  {t('backendConfig.effortMinimal')}
                </option>
                <option class="select-option" value="none">
                  {t('backendConfig.effortNone')}
                </option>
              </select>
            </label>
            <label class="field-label">
              ...
              <select
                ...
                onChange={(e) =>
                  setOpenrouterReasoningSummary(e.currentTarget.value as AppSettings['openrouter.reasoningSummary'])
                }
              >
                <option class="select-option" value="">
                  {t('backendConfig.optionDefault')}
                </option>
                <option class="select-option" value="auto">
                  {t('backendConfig.summaryAuto')}
                </option>
                <option class="select-option" value="concise">
                  {t('backendConfig.summaryConcise')}
                </option>
                <option class="select-option" value="detailed">
                  {t('backendConfig.summaryDetailed')}
                </option>
              </select>
            </label>
          </div>
        ...
        <Show when={backendProvider() === 'claude' || backendProvider() === 'openrouter'}>
          <div class="mt-md flex-col-sm">
            ...
            <label class="field-label">
              ...
              <select
                class="select"
                data-testid="backend-config-cache-mode"
                value={cacheMode()}
                onChange={(e) => markDirty(setCacheMode)(e.currentTarget.value)}
              >
                <option class="select-option" value="off">
                  {t('backendConfig.cacheModeOff')}
                </option>
                <option class="select-option" value="auto">
                  {t('backendConfig.cacheModeAuto')}
                </option>
                <option class="select-option" value="manual">
                  {t('backendConfig.cacheModeManual')}
                </option>
              </select>
            ...
            <Show when={cacheMode() === 'manual'}>
              <label class="field-label">
                ...
                <input
                  class="input"
                  type="number"
                  data-testid="backend-config-cache-depth"
                  min={0}
                  max={100}
                  value={cacheDepth()}
            ...
            <label class="field-label">
              ...
              <input
                class="input"
                type="text"
                data-testid="backend-config-cache-ttl"
                value={cacheTTL()}
      ...
      <div class="modal-actions">
        <button class="btn" data-testid="backend-config-close" onClick={close}>
          {t('common.close')}
        </button>
      </div>
    </Modal>
