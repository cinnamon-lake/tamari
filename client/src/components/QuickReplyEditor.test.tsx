...
describe('QuickReplyEditor', () => {
  ...
  it('renders in create mode', () => {
    render(() => <QuickReplyEditor chatId="chat-1" characterId="char-1" onClose={() => {}} />);
    expect(screen.getByText('New Quick Reply')).toBeInTheDocument();
  });

  it('renders in edit mode', () => {
    const qr = makeQR({ id: 'qr1', label: 'Greet', script: 'st.send("Hi")' });
    render(() => <QuickReplyEditor qr={qr} chatId="chat-1" characterId="char-1" onClose={() => {}} />);
  ...
  it('sends quickreply.create on save in create mode', () => {
    ...
    const onClose = vi.fn();
    render(() => <QuickReplyEditor chatId="chat-1" characterId="char-1" onClose={onClose} />);
    ...
    expect(sendSpy).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        patch: expect.objectContaining({
          autoExecute:
            QuickReplyAutoExecute.USER_MESSAGE | QuickReplyAutoExecute.AI_MESSAGE | QuickReplyAutoExecute.STARTUP,
          orderIndex: 3,
        }),
      }),
    );
