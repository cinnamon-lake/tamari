...
describe('TransformerChainsModal', () => {
  ...
  it('creates a chain via transformerchain.save without an id', () => {
    const sendSpy = vi.spyOn(bus, 'send').mockImplementation(() => {});
    render(() => <TransformerChainsModal onClose={() => {}} />);

    fireEvent.click(screen.getByText('Add Chain'));
    fireEvent.input(screen.getByPlaceholderText('my-chain'), { target: { value: 'My Chain' } });
    fireEvent.click(screen.getByText('Save'));

    const saves = sentSaves(sendSpy);
    expect(saves).toHaveLength(1);
    expect((saves[0] as { id?: string }).id).toBeUndefined();
    expect((saves[0] as { data: unknown }).data).toEqual({ name: 'My Chain', description: '', steps: [] });
  });
  ...
  it('appends a builtin step with default params and a param form', () => {
    const sendSpy = vi.spyOn(bus, 'send').mockImplementation(() => {});
    render(() => <TransformerChainsModal onClose={() => {}} />);

    fireEvent.click(screen.getByText('Add Chain'));
    fireEvent.input(screen.getByPlaceholderText('my-chain'), { target: { value: 'C' } });
    ...
    const saves = sentSaves(sendSpy);
    expect(saves).toHaveLength(1);
    expect((saves[0] as { data: { steps: unknown[] } }).data.steps).toEqual([
      { kind: 'builtin', id: 'whitespace', enabled: true, params: { mode: 'full' } },
    ]);
  });

  it('appends a Lua script step from the scripts list', () => {
    const sendSpy = vi.spyOn(bus, 'send').mockImplementation(() => {});
  ...
  it('edits a chain via transformerchain.save with the id, toggling step enabled', () => {
    const sendSpy = vi.spyOn(bus, 'send').mockImplementation(() => {});
    setState('transformerChains', [
      makeChain({
        steps: [
  ...
  it('reorders steps with the move buttons', () => {
    ...
    fireEvent.click(screen.getByText('Save'));
    const saves = sentSaves(sendSpy);
    expect((saves[0] as { data: { steps: Array<{ id?: string }> } }).data.steps.map((s) => s.id)).toEqual([
      'strip-reasoning',
      'squash-system',
    ]);
  });
  ...
  it('edits history-squash params and clears a prefix back to the default', () => {
    const sendSpy = vi.spyOn(bus, 'send').mockImplementation(() => {});
