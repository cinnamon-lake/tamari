...
5. **Response post-processing is forced off** — `trimSentences`, `removeXML`, `singleLine`: persisted assistant text must be the raw provider stream, byte for byte.
6. **Reasoning is always re-sent verbatim** — the provider's snapshot includes the reasoning it generated. (The old `reasoningAddToPrompts` forcing is gone with the setting: the renderer never strips anymore; stripping is exclusively the strip-reasoning request transformer's job.)
...
9. **Volatile-but-wanted content hoists** to pinned system messages at the top of history (after the prompt-list head, before message 1) — one message per item, never joined — deterministic order: author's note text → constant atDepth WI → absolute-position preset prompts. They change only when their inputs change (note edit, constant-set change) — an occasional documented re-warm, not a per-turn bleed. Hoists are trace-noted, silent in the UI.
...
- Generation pipeline: `trimSentences` / `removeXML` / `singleLine` / output-regex forced off, memory summaries skipped, and **request transformer chains disabled wholesale** (no chain is resolved; the `requestTransformers` stage no-ops) — all resolved once per build via `appendOnlyLocks.ts` (`resolveEffectiveSettings`).
