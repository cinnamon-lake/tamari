...
| **Text-completion story string templates** | SillyTavern had a completely separate template system for text-completion APIs (`renderStoryString`, `power-user.js`). | ❌ **Intentionally omitted.** `PromptManager` handles prompt assembly uniformly. Text-completion adapters flatten the assembled message list into a single string via their `InstructTemplate` (`backends/formatTextPrompt.ts`). No separate template system. |
...
| **Prompts ordered after the `chatHistory` marker** | The renderers used to ignore the Chat History marker's position: history always rendered after ALL relative prompts, so anything ordered after it (e.g. Post-History Instructions / jailbreak) actually rendered *before* the history. | ❌ **Intentionally changed (bug fix).** Prompts before the marker render before the history, prompts after it render after — the preset editor's ordering is now honored in both directions. Author's Note in `after_prompt` position likewise lands after the history (matching its name and ST semantics). Only presets with non-empty content ordered after Chat History are affected; defaults are unchanged. |
| **Disabling the `main` prompt** | `getOrderedPrompts()` used to emit `main` even when its order entry was disabled, making the Main Prompt enable checkbox in the preset editor a no-op. | ❌ **Intentionally changed (bug fix).** The checkbox is honored: a disabled `main` is excluded from the prompt like any other entry. Affects only presets where Main Prompt was explicitly disabled (previously ignored). Character System Prompt overrides apply to the `main` slot, so they are excluded with it. |
