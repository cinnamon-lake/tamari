...
**Debug traces.** A successful `run_agent` result ends with a `[trace: <generationId>]` reference, and a failed one renders the composed ancestry chain (which layer died, after how many rounds and which tool calls) instead of a bare error string. The id opens read-only in the workbench at `/generations/<id>/` (`meta.json`, `error.txt`, and `prompt.json` when the `debugPrompts` setting was on). A sub-agent tool that failed but let the run recover leaves a `warnings` note on the result.
