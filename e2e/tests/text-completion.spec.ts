...
test.describe('Text Completion Mode', () => {
  ...
  test('text mode sends a flat instruct prompt to /completions and renders the reply', async ({ page, app }) => {
...
test.describe('Text Completion Mode — renderer and adapter edges', () => {
  // Requests `app` so the fixture's login + configureMockBackend run BEFORE
  // the generationMode patch below (fixtures are lazy otherwise).
  ...
  test('custom instruct template markers wrap the flat prompt (bos/eos/responsePrefix)', async ({ page, app }) => {
  ...
  test('continue in text mode sends the prior reply as raw prefill', async ({ page, app }) => {
  ...
  test('maps a length finish reason in text mode', async ({ app }) => {
