...
Summaries keep the context clean — tagged blocks whose closing tag carries the gist, so history outgrows the window gracefully.
...
**Structural tags are player-facing renderings; cast is state.** The \`[event kind]\` open and the \`[/event kind summary="..."]\` close exist for the player's plot log (display rules hide the open, render the gist) — nothing parses them, and they are strictly optional. Who is on stage is NOT a tag: it rides the newest user message as a parenthetical cast note, built from \`state.event.participants\` (\`ev.castLine()\`) — volatile state in the newest message, never the frozen prefix. The model never types a bracket: the script emits every tag and strips freelanced ones from delegate text.
...
## Compaction: summary tags and the zoom chain

History is what outgrows the context window first, and the answer has two halves that never meet. For the PLAYER, spans are marked in the log with tagged blocks whose CLOSING tag carries the gist — stored history is immutable, so the gist can only ride the close tag (or \`state\`), never be spliced back into the opening message:
...
A display rule hides the open and renders the close's gist as a plot-log line — the player gets the compressed memoir of what they lived. These tags are STRICTLY OPTIONAL: nothing parses them in either direction (packs resolve through the store, spans are mechanical, memory is rolling), so a card that emits none of them runs identically — they exist because the plot log is nice to read. (HUD/MAP tags are a different animal: functional chrome the display rules render, not memoir.) For the MODEL, memory is not a folded view of history at all: it is the rolling story channel (The game lib) — the same gist, filed mechanically with the span it covers as zoomable content, briefed as \`STORY SO FAR\`, inspectable by id. No delegate ever reads raw history, so there is nothing left to collapse FOR one. (The open tag is still emitted, and the pair still matters: interleaved blocks are how the plot log knows where each block starts and ends.)

**Who splices the summary:** the script, on the closing turn — a gist sub-gen over the span (the mechanical fight log, or \`summarize.span\` over the open tag for cards that track spans in the log), spliced deterministically into the close tag the script appends. Script owns the format (no double quotes inside summaries), the model owns the content. A close tag arriving with NO summary means the model freelanced: derive one or strip the tag — never leak it. This whole recipe ships as \`lib/summarize\` (The game lib): \`open\`/\`gist\`/\`close\`/\`fixClose\`.

- **Gist vs exact.** A summary is a paraphrase. Gist lives in tags and story entries, mechanics in \`state\`, commitments in the ledger — a fact that must stay exact may never travel ONLY in a summary.

**The zoom chain is the antidote.** Once the delegate's view is compressed, give it a way back to the record — recursively, not with a text search. The fight log and the event span are filed as rolling entry CONTENT (mechanical, branch-aware — never re-parsed), and \`inspect_summary({ id })\` (lib/rolling) opens any summary by id: a fold entry lists the summaries inside it, each with its own id, down to the raw blows and chat blocks. The model tool-calls its way from the digest to the exact exchange it half-remembers. One honest bound: summaries and spans are text, not truth — how the goblin encounter WENT is in the story, the goblin's HP is in \`state\`. Exact facts still belong to registry/ledger. (The \`chat\` global's full-branch \`chat.find\` remains as a raw escape hatch, topic \`custom_backends\` — the zoom chain is the structured answer.)

Four channels, four fidelity levels: \`state\` for mechanics, the ledger for commitments, tags for gist, the store for verbatim-on-demand (rolling content, spans, packs — ids in \`state\`, blobs in the heap).
...
8. **Regexes last.** Add the display rules that style what the script emits — the HUD panel, the plot-log line off a close tag, the MAP render, hidden command messages. Last, because every rule depends on a tag the script already emits.
