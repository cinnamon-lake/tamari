...
local sub = {
  ...
  messages = {
    { role = "user",      content = "I poke the rusted door" },
    { role = "assistant", content = "It doesn't budge. The rust flakes, but the frame holds." },
    { role = "user",      content = "fine. I wedge the bomb into the hinge gap and run" },
  },
  tools = escalationTools:schemas(),  -- attempt, remove_item, add_exit, spawn_enemy, grant, ...
}
...
For a novel action — something the deterministic state machine has no rule for — the delegate is the game master. The player blows up the door with a bomb, and the delegate decides that works and performs it through tool calls: \`remove_item({ name = "bomb" })\`, \`add_exit({ to = "r4", via = "destroyed door" })\` — a passage to an ADJACENT room id, with the compass labels derived from the grid geometry — maybe \`spawn_enemy({ name = "gutter troll" })\` for the noise it made. Lua runs those calls and holds the invariants: you can't spend a bomb you don't have, a bribe can't exceed the purse, a blown wall connects orthogonal neighbors only. That is all Lua does with a mutation. It does not decide what the action costs — it can't; there is no rule table that knows what "I blow up the door with a bomb" means, and writing one means enumerating every action a player could invent.
...
Chat is the state for the parts that aren't mechanics: actual conversation. The entry is the script failing to parse the input as a command, so it builds a prompt — some context (probably the summary of the last delve), the recent turns, the user's text as a plain \`{"role":"user","content":…}\` — and forwards the model's reply. Within a chat scene the prompt is append-only — turn N is a strict prefix of turn N+1 — so the delegate's prefix cache hits and a per-turn call stays cheap (the span mechanics live in the event-engine section below).
...
Summarize the situation that opened the scene INTO that context before the mode flips — what just happened AND the relationship state (first meeting or known) — because node zero of the span is written when the event opens, and anything it missed can only arrive appended to a LATER node at a partial-cache cost (see the span below). The open context is its picture of how we got here; the public record (STORY SO FAR in its briefing, \`inspect_summary\` in its toolset — composed separately, via \`ts:use(rolling)\`) and the dossiers are how it checks that picture.
...
while phase do
  if phase == "dm" then
    ...
    reply = res.text                          -- DM framing: the reply only if NO event opened
  elseif phase == "scene" then
    ...
    -- the prompt is the span plus the new input; the loop's tool rounds
    -- accumulate on sceneSub.messages, and everything this turn added goes
    -- onto the span in ONE append AFTER the loop (that is what makes the
    -- tail full-fidelity: user node, tool_use/tool_result rounds, reply)
    sceneSub.messages = concat(ev.span(), { { role = "user", content = input } })
    local res = loop.run(sceneSub, backends.generate(sceneSub):await(), tools:exec())
    ev.spanAppend(nodesAddedThisTurn)
...
1. **\`open_event\` returns immediately.** The DM frames, files, and stops talking. Its prose is the reply only when NO event opened; once the scene opens, the scene's opening line wins, and a scene-gen ERROR fails the turn per Failure UX (the branch bricks) — the DM framing is not a fallback for a broken scene.
...
Volatile state still rides in the newest message, never deep in the span; anything that would otherwise mutate early nodes — character defs, dossiers — arrives as READ-tool results in the tail instead. Branch correctness comes free (an old branch's \`spanId\` still points at its own head), and history budgets are irrelevant — the span never touches the log. The Guildhall's tests assert the strict prefix across turns AND that turn 2's tail carries turn 1's tool blocks.
...
When a character reappears, \`get_character\` serves their dossier as a read-tool result — recent few in full, and when the backlog outgrows the window, one cheap sub-gen FOLDS the oldest takes into a running digest. Dossiers are \`lib/rolling\` channels (\`state.dossiers[charId]\` — takes are gist-only entries pushed to the log half; the character's kv facts live in their registry record, not here). One honesty note on the serve shape: it is gist text only — recent takes in full, then the running digest, plus an \`older_takes\` count of how many fold entries sit behind it — and it carries NO entry ids, so the zoom chain does not reach into a dossier; \`inspect_summary\` zooms the story channel, whose briefing does serve ids. The fold runs on READ, so one-off NPCs never cost a token; a delegate error fails the turn loudly (see Failure UX — recovery is a swipe, and ids move only once the fold entry is filed, so the retry sees memory intact).
...
- **One tool: \`promise({ id, what, due })\`** (plus \`resolve_promise\`). Filed mid-narration like any registry call. The critical validation: \`due\` must be a CONCRETE anchor — an ABSOLUTE number in the card's own clock (turn 40, floor 12, week 20), never "later"; a vague due date is a promise that never comes due. The lib enforces this by type: \`due\` is an integer in whatever units \`ledger.bind\`'s fn reports as "now" (a card that thinks in floors binds a floor counter), and a filed \`due\` is clamped to now+1 … now+50 — never this turn, never past the horizon. Non-numeric anchors (an event id, a stat threshold) don't fit the lib's clock; express them as card-side executable-tier records instead (next bullet).
...
- **The briefing is the memory.** The pending ledger rides in every briefing; Lua computes due-ness against the bound clock (the lib knows only the number your \`ledger.bind\` fn returns — floors, dates, and stats are the card's choice of clock) and escalates — \`DUE NOW: the brother — design him this turn\`. Escalation is what makes "I'll finalize later" reliable instead of a prayer.
...
**Who writes the summary:** the model, on the closing turn — \`lib/summarize\`'s gist sub-gen over the span the card tracked mechanically (the fight log in \`state\`). The script owns the discipline (one line, no double quotes — enforced by post-processing, not just prompted), the model owns the content. A nil means there was nothing to summarize (no span, empty answer) — the card serves its fallback line; a delegate ERROR propagates to the card, which decides: fail the turn per Failure UX, or pcall and serve a canned line (a fight gist isn't worth bricking a branch over).
...
The reusable 90% of this architecture — the parts every game card re-derives, some of them genuinely tricky (the persistent-array span, the js_null footgun: \`json.decode\` returns a sentinel object for JSON null, not Lua nil, so naive field checks lie) — ships as twelve small Lua modules. Get them into a card with one call — \`run {"verb":"add_game_lib","args":{"characterId":"…"}}\` vendors all twelve as \`backend_logic/lib/*.lua\` (overwriting \`lib/\` keys only; topic \`workbench\`) — then \`require("lib/<name>")\`. The full sources are at the end of topic \`game_cards_example\`; the Guildhall is the single worked example. Vendored, not engine-provided: the card owns its copies, so exports work on any install and behavior is pinned per card.
...
local enemies = registry.new({
  ...
  -- for planning gens that speculate before committing. Does NOT combine with
  -- partition_by (a construction error) — partitioned writes ARE the commit path.
...
**The card-side surface.** A registry instance is also the card's own query layer (plain dot calls): unpartitioned — \`list()\`, \`get(id)\`, \`create(fields)\`, \`update(id, fields)\`; partitioned — \`list(pk)\`, \`get(pk, id)\`, \`create(fields)\` (the partition is derived from the record), \`update(pk, id, fields)\`; plus \`all()\` (cross-partition) and \`briefing(pk?)\` (one line per record, for prompt sections). \`create(fields)\` returns the id — or \`id, "already_registered"\` on convergence, or \`nil, reason\` on rejection — and \`list()\` with no key on a PARTITIONED registry throws; use \`all()\` there. Unpartitioned records are LIVE — mutate in place and every consumer sees it; partitioned reads return resolved copies, so mutate those through \`update\`. Custom read shapes declare as \`queries = { { name, args, run } }\` — each becomes a tool (schema built from \`args\`) AND a card-side method of the same name, with \`run(records, args)\` receiving the full cross-partition record list. Registries that should share one pack per area share a \`packs_key\` (default \`"packIds"\`).

**Partitioned registries: packs as the persistence unit.** A partitioned registry's records stop living at \`state.enemies\` and start living in PACKS — one store blob per area, shared across all partitioned registries, with \`state\` holding only the pointer (\`state.packIds[areaId]\`). The partition is a property OF THE RECORD, derived by the card, never seen by the model: declare \`partition_by = "floor"\` — a FIELD NAME, preferred whenever the routing field is a real record field — and the registry routes each write itself: a monster's partition is the floor it spawns on, a floor layout's partition is that floor, an item's is \`global\` because it's needed everywhere. Budgets go per-partition here: a partitioned \`cap = 8\` means 8 records in EACH area, and the rejection names the partition. (A function form, \`partition_by = function(rec) return rec.floor end\`, routes the same but leaves the routing field anonymous — see the next paragraph for what that costs.)

With the string form, the query and update tool schemas ask for the routing field by its DOMAIN name — \`get_enemy({ id, floor })\` requires a floor, because that's the sensible way to ask — and every lookup lands in exactly one partition (a query miss says \`unknown enemies: x in f2\` — the registry's \`key\`, verbatim, never singularized). "Partition" as a concept never appears in a schema: the interface speaks fiction (which floor, which room), the storage layer reads the same field for routing. Exposing the loading mechanism as such would be one more thing for the delegate to fumble, and it already told you where the trog goes — \`location = "upper"\` IS the partition, the registry just reads it. With the FUNCTION form the routing field is anonymous, so model-facing lookups must scan every partition instead — and an id found in more than one is a loud ambiguity rejection, never a silent first-match guess. The escape hatch: declare \`partition_field = "floor"\` alongside a function \`partition_by\` and the schemas get the named, single-partition lookup anyway (the field must be a real declared field; the function still decides routing).

Writes update the in-memory view immediately and queue a mutation record keyed to the derived partition; the card calls \`registry.flush()\` once at the end of \`generate()\` — queued mutations applied per pack, one new \`store.putJson\` per touched pack, the pointers move. Reads (\`get_enemy\`, the handlers' internal lookups) lazy-load the relevant packs through the pointer table and resolve base-plus-mutations; cross-area reads just work, because loading is the script's problem, not the model's. Unpartitioned registries work exactly as before — they're the \`partition_by\` returning \`global\` case, conceptually — and the planning gen and the escalation delegate use the SAME mutation tools with no awareness of any of this: one tool economy, and the routing is derived from the fiction itself.

**Mutable fields get set semantics.** Registry records are non-compacting information (The two memory kinds): some fields are fixed at creation (a name, a concept), but others legitimately EVOLVE — a character's appearance, their personality summary, their status. Declare those as \`mutable = { "appearance", "personality" }\` and the lib emits an update tool (\`update_character({ id, appearance = ... })\`) that OVERWRITES the listed fields on the existing record — same validation, same clamps, id stable, latest value canon. The update tool's name derives from the mutation tool's (\`register_character\` → \`update_character\`); a mutation tool not named \`register_*\` falls back to \`update_<key>\` — so \`tool = "add_item"\` with \`key = "items"\` yields \`update_items\` — or name it yourself with \`update_tool\`. (This is the set-vs-append rule from The two memory kinds, applied to records: the dossier is where history goes; the record is where the present lives.)

### Module index (the utilities)

- **\`loop\`** — the delegate tool loop: \`loop.run(sub, res, exec, maxRounds?, opts?)\`, default cap 16 (a todo-planning delegate eats rounds on top of its real calls); hitting the cap with calls still pending THROWS — a wedged delegate fails the turn loudly, it never silently drops pending tool work. Two escape hatches via \`opts\`: \`opts.done\` (a zero-arg predicate checked before each round — the loop stops early, pending calls cleared) and \`opts.soft\` (hitting the cap RETURNS the last result instead of throwing — pending calls cleared the same way; the caller owns the fallback; \`lib/events\` finalizes this way, so a capped close still lands its gist).
- **\`sanitize\`** — \`sanitize.data(t)\`, strips js_null and non-data from \`json.decode\` output. Mind the aliasing contract: an ARRAY argument is rebuilt (a fresh table comes back, the input untouched — always use the return value) while a MAP argument is mutated in place.
- **\`chrome\`** — \`chrome.btn(cmd, label)\` (bare command payload), \`chrome.unwrap(text)\` (a posted command → bare verb), \`chrome.clean(text)\` (the deterministic strip for anything that reaches a delegate: legacy \`[sys]\`, buttons, HUD and MAP tags, trim — the CARD applies it to user input before \`ev.spanAppend\`, which does no cleaning itself, and \`inspect\` rendering uses it), \`chrome.oneline(text, max?)\` (one tag-safe line: quotes, whitespace; the length cap is opt-in, for excerpts like the zoom chain's inspect rendering — filing channels never cap).
- **\`ledger\`** — the plot ledger as a module: \`ledger.bind(fn)\` once per turn (\`function() return state.turn end\` — the fn is what "now" means for due-ness clamps), then \`tools()\`/\`exec()\`/\`briefing()\`. Set semantics are strict: re-filing an existing id REPLACES the entry and reopens it as pending (a re-filed "resolved" promise can't strand a shadow duplicate), and \`resolve_promise\` takes \`outcome\` as a closed enum (\`"kept"\`/\`"failed"\` — anything else rejects).
- **\`todo\`** — delegate self-planning, for sub-gens whose job spans many tool calls (pack planning, multi-step escalations): \`set_todo\` REPLACES the checklist (a missing/non-array \`items\` rejects instead of silently erasing the plan), \`todo_done\` marks items by 1-based index (idempotently — an already-done item says so, an out-of-range index rejects), every result echoes the remaining list so the plan rides the tool loop.
- **\`summarize\`** — the gist engine: \`summarize.gist(prompt, opts?)\` runs the gist sub-gen over \`opts.span\` (the card's mechanically tracked span — the fight log; mechanical turn-log in, "how it WENT" line out — costs, close calls, items spent; non-string span entries are skipped, and the result is post-processed into the one-line/no-double-quotes discipline rather than trusting the prompt). \`opts.maxSpanChars\` (default 6000) budgets the span newest-first, \`opts.instructions\` steers the angle. Nil when there's nothing to summarize — no span, an empty answer, or no line fit the budget; a delegate error PROPAGATES to the caller, who decides — fail the turn per Failure UX, or pcall and degrade to a canned line (the Guildhall's fight gists do). The gist goes two places, both tagless: a plain memoir line in the reply and a rolling story entry.
- **\`maptag\`** — \`maptag.tag(rooms, { cur, entrance, stairs, seen? })\` builds a compact \`[MAP|…]\` tag from a room graph; a companion display rule renders it (source in \`game_cards_example\`). Two output shapes, decided per GRAPH: when every room carries numeric \`x\`/\`y\` (lib/layout's shape) the tag is a coordinate grid — \`id=x,y,Name\` rooms normalized over the whole graph so the map doesn't drift as fog lifts, undirected \`a-b\` passages — and ONE coord-less room flips the whole tag to the legacy direction-labeled form. \`seen\` is the fog of war: seen rooms get real names, rooms adjacent to the seen set render as \`?\` (the frontier), everything else is omitted; \`seen = nil\` reveals the whole graph, \`cur\` is always shown, and the stairs marker stays hidden until the stairs room itself is seen. Names and labels are stripped of tag metacharacters (\`| ; > [ ] = < ' " & , -\`) — punctuation in a room name silently vanishes from the map.
- **\`toolset\`** — composition (see contract snippet above). Duplicate tool names are a hard composition-time error, so overlapping modules fail loudly at \`ts:use\`, not at call time.
- **\`events\`** — the event engine from the section above. The card creates the character registry itself (\`registry.new\` with ITS fields) and injects it — \`events.new({ roster })\` (or declare \`{ fields, key? }\` and the engine creates the roster) — so the cast stays SHARED, never opaque (another toolset gets the same instance; \`roster.get(id)\` returns the live record for ad-hoc mutations like \`rec.dead = true\`). An injected roster is checked at construction: its fields must not collide with the engine's reserved ones (\`digest\`/\`dossier\`/\`older_takes\` — the keys \`get_character\` injects into its result), and it must be UNpartitioned (the engine looks members up one-arg). Note the dispatch order: the roster's tools are tried FIRST, so a card-declared roster tool or query named \`get_character\`/\`list_characters\`/\`add_to_chat\`/\`close_event\` would silently shadow the engine's. The engine owns event state (\`isOpen\`/\`kind\`/\`eventLine\`/\`clear\`), the cast tools (\`register_character\`/\`list_characters\`/\`get_character\`/\`add_to_chat\`), dossiers with fold-on-read digestion — armed by \`ev.bindPrompt(prompt)\` once per generate, deferred to the next bound generate otherwise — no structural tags (\`ev.strip\` as pure defense, applied to delegate text before serving), the cast note (\`castLine\` — serves each participant's display NAME, falling back to the id), the append-only span (\`spanStart\`/\`spanAppend\`/\`span\`/\`hasSpan\` — guard them: \`spanStart\` needs an open event, \`spanAppend\` needs a started span, and \`span()\` returns \`{}\` when neither exists), and \`finalize(prompt)\` for the deterministic \`/leave\` (soft-capped at 4 rounds: a finalizer that never closes still lands the fallback gist, and an over-eager one that re-calls \`close_event\` after the close gets a terminal "already closed" success, not a brick). \`close_event\` validates every take BEFORE filing any — a bad take rejects the call, it never bricks the branch mid-close. One toolset, two views by mode: the DM escalates with the full toolset and the scene-runner runs with the same one — tools that don't fit the current mode (a second \`open_event\`, a DM-side \`close_event\`) fail as ordinary error results. The engine's own toolset does NOT include \`inspect_summary\` — compose it in with \`ts:use(rolling)\` where the zoom is wanted.
- **\`layout\`** — procedural floor/area layouts, Lua-decided: \`layout.generate({ rooms?, sections?, loops?, sprawl?, terminal? })\` grows a connected blob of cells on an integer grid (orthogonal adjacency only, so the map is planar by construction), partitions it into contiguous lettered sections, spans it with a passage tree plus \`loops\` extra connections, and picks an entrance (a degree-1 border cell where one exists — a ring-shaped blob has none, and then any cell can be picked) and stairs (the farthest room; nil when \`terminal\`). Knobs clamp: \`rooms\` 4–24 (default 8), \`sections\` 1–4 (default 2), \`loops\` 0–6 (default 1), \`sprawl\` 0–1 (default 0.5). The return is \`{ order, rooms, edges, entrance, stairsDown, sections, deadEnds }\`: rooms come back with \`x\`/\`y\`/\`section\` and symmetric compass-labeled exits — the shape \`maptag\`'s grid mode consumes — and the stairs room also gets an \`exits.down = "down"\` pseudo-exit, the serve path's descent trigger (\`maptag\` ignores it: "down" is no room). NOT deterministic (math.random + hash-order tie-breaks); the guarantees are structural, not reproducible. \`layout.skeleton(lay)\` renders the layout as the text block a planning sub-gen themes from (section grid, room lists, passages, dead ends). The point: topology is a generation problem models are bad at and Lua is exact at — let Lua own it, and the model's whole job narrows to theme.
