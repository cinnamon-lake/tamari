...
A complete, TESTED factory-ratio card (\`backend_logic/main.lua\`): three floors of room-graphs, a relic, and a model that authors content in bulk instead of narrating every turn. Theory lives in topic \`game_cards\` (The content factory); this topic is the steal-able file. (Repo copy \`docs/design/examples/sunken-crypt/main.lua\`, validated end-to-end through the real adapter by \`server/src/backends/sunkenCrypt.example.test.ts\`.) Decisions worth noticing:
...
- **Scale is one constant.** \`ROOMS_PER_FLOOR\` is "6-10" here so the example stays readable — a real descent game wants "24-40". The planning sub-gen is paid ONCE per floor either way, and serve turns are free at any floor size. (Big floors also want \`runLoop\`'s round cap raised — the Crypt passes 12 for planning.)
- **Packs live in the LOG as append-only tagged blobs — never in \`state\`.** \`state\` snapshots persist per message; bulk packs would duplicate horribly. \`findPack\` reads the newest blob back via \`chat.find\` (newest-first), so pack mutation is a NEW blob in the same turn's message, not an edit. The blob's close tag carries a script-composed summary, so \`collapseBlocks\`-style compaction and plot-log display rules work unchanged. \`state.room\` is \`"f2:r5"\` — floor : room — so flags key off it for free (\`used:f2:r5:crate\`, \`quiet:f2:r5\`).
...
- **The DM sees a RECENT TURNS transcript** — chrome-stripped, capped, and block-collapsed by \`collapseBlocks\`: pack blobs arrive as their one-line summaries ("Designed The Upper Halls: …"), not kilobytes of JSON. Emission and collapse in one card.
- **\`cleanNulls\` is load-bearing.** \`json.decode\` maps JSON \`null\` to a truthy js_null userdata, NOT Lua nil — \`if pack.encounter then\` takes the wrong branch and concatenating it errors. Sanitize decoded JSON before use.

Companion character-scoped regex rules (\`write /characters/<id>/regex/new.json\`): \`/\\s*\\[sys\\].*?\\[\\/sys\\]\\s*/gis\` → \`"\\n\\n"\` (hide \`[sys]\` acks), optionally \`/^\\s*\\/\\w+.*$/s\` with role \`userInput\` → \`""\` (hide whole-message commands — safe because posted commands are bare text with no HTML to mangle), \`/\\[HUD\\|([^\\]]+)\\]/g\` → panel HTML (topic \`regexes\`), and \`/\\[pack (\\w[\\w ]*)\\][\\s\\S]*?\\[\\/pack \\1 summary="([^"]*)"\\]/g\` → \`<div class="plot-log">$2</div>\` (pack blobs render as plot-log lines). Dry-run with the workbench \`test_backend_logic\` verb — its \`history\` option is how you feed canned pack blobs to \`chat.find\`.
