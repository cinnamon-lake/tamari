...
- **Where resolved:** preset prompts, character card fields, persona, World Info at-depth content, Author's Note, runtime injections, stopping strings — all at prompt-build time. Chat messages are resolved once **at write time** and stored resolved (so \`{{random}}\` in a sent message is frozen; prompts re-roll per generation).
