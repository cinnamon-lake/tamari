...
| \`history-squash\` | \`role\`: \`user\` (default) / \`assistant\`; \`userPrefix\`?, \`userSuffix\` (\`''\`), \`charPrefix\`?, \`charSuffix\` (\`''\`) | Collapse all user/assistant turns into ONE message of the target \`role\`, placed at the first collapsed turn's position. Covers the classic user-squash extension (\`role: 'user'\`) and noass (\`role: 'assistant'\`) — the same transform differing only in role. Each turn is wrapped in its per-role prefix/suffix (prefixes default to \`<userName>: \` / \`<charName>: \` from the resolved names) and becomes its own text part in the collapsed message — no joining; adapters own wire-format flattening. System and tool messages stay untouched in place (preamble stays preamble, jailbreak stays last); textless user/assistant messages are left alone. NoAss's advanced post-processing (history cropping, rearranging, inter-split prompts, \`{{lastlines}}\`) is out of scope — a Lua step can do it. Incompatible with prompt caching: the whole history block is rewritten every turn. |
...
- \`whitespace\` BEFORE \`history-squash\`: clean the text first, then wrap it, so no stray runs survive inside the collapsed block.
