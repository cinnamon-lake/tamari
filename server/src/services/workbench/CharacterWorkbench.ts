...
import {
  ...
  mergeRegexRules,
  getGlobalRegexRules,
  CHARACTER_REGEX_EXTENSION_KEY,
} from '../characterRegex.js';
...
const log = getLogger('services/workbench/CharacterWorkbench');
...
const CharacterCloneArgs = z.object({
  ...
  name: z
    ...
    .max(512)
    .optional()
    .describe('Name for the clone. Defaults to "<source name> (Copy)". Must not collide with an existing character.'),
});
  ...
  patch: z
    .object({
      name: z
        ...
        .max(512)
        .optional()
        .describe('Rename the character. Fails if another character already has that name.'),
      ...characterFields,
    })
...
const CharacterSetAvatarArgs = z.object({
  ...
  sourceCharacterId: z
    ...
    .optional()
    .describe(
      "Copy the avatar from this character's card instead of using an attachment. Pass exactly one of attachmentId / sourceCharacterId.",
    ),
});
...
const RegexTestArgs = z.object({
  characterId: z
    ...
    .optional()
    .describe(
      'Character whose scoped rules to include (always merged after global rules). Omit to test global rules only.',
    ),
  text: z.string().max(100_000).describe('Sample text to run through the rules.'),
  role: z
    .enum(['user', 'assistant'])
    .optional()
    .describe('Role to test as, for role-filtered rules. Default assistant.'),
});
...
const RisuModuleGetArgs = z.object({
  ...
  index: z
    ...
    .min(0)
    .optional()
    .describe('Trigger index (from risu_module_get section=triggers). Required for section=trigger.'),
});
...
const CharacterAssetAddArgs = z.object({
  ...
  attachmentId: z
    .string()
    .describe('Attachment id of the file to import as a character asset (e.g. generated by a media tool).'),
  name: z.string().min(1).optional().describe('Asset name. Defaults to the attachment id.'),
  type: z
    .string()
    .optional()
    .describe(
      'Asset type (e.g. image, audio, video, other). Defaults to a value derived from the attachment MIME type.',
    ),
});
...
const BackendLogicGetArgs = z.object({
  ...
  offset: z
    ...
    .min(1)
    .optional()
    .describe('First source line to return (1-indexed). Omit for the full source.'),
...
const BackendLogicSetArgs = z.object({
  ...
  luaSource: z
    .string()
    .optional()
    .describe('Lua source implementing generate(prompt, ctx). Omit to keep the current script.'),
  enabled: z
    .boolean()
    .optional()
    .describe('Activate/deactivate the card logic. Omit to keep the current state. Default for new scripts: false.'),
});
...
const BackendLogicTestArgs = z.object({
  ...
  luaSource: z
    .string()
    .optional()
    .describe("Test this Lua source instead of the character's stored script — iterate without saving."),
  state: z
...
const BackendFilePathArgs = z.object({
  ...
  path: z
    .string()
    .describe(
      'Module path inside backend_logic/ (e.g. lib/utils.lua). Slash-separated; the .lua extension is appended when omitted.',
    ),
});
...
const BackendFileSetArgs = z.object({
  ...
  luaSource: z
    .string()
    .describe("Lua module source. The chunk's return value is the module; top-level return is allowed."),
});
...
export class CharacterWorkbench {
  ...
  private async characterClone(args: Record<string, unknown>): Promise<ToolExecuteResult> {
    ...
    if (source.avatarPath && this.deps.storage.exists(source.avatarPath)) {
      avatarPath = this.deps.storage.write(
        'avatars',
        `${randomUUID()}.png`,
        new Uint8Array(this.deps.storage.read(source.avatarPath)),
      );
    }
    if (source.avatarThumbnailPath && this.deps.storage.exists(source.avatarThumbnailPath)) {
      avatarThumbnailPath = this.deps.storage.write(
        'avatars/thumbs',
        `${randomUUID()}.png`,
        new Uint8Array(this.deps.storage.read(source.avatarThumbnailPath)),
      );
    }
  ...
  private async broadcastBook(kind: 'created' | 'updated', book: WorldInfo): Promise<void> {
    this.deps.ragService
      ?.indexWorldInfoEntries(book.id, book.entries)
  ...
  private async resolveLorebookEntry(
    ...
    if (!book)
      return {
        error: `Error: character "${characterId}" has no lorebook yet — add an entry with lorebook_entry_add first`,
      };
  ...
  private validateFindRegex(findRegex: string): string | null {
    return parseRegexString(findRegex)
      ? null
      : `Error: invalid findRegex "${findRegex}" — use JS-style delimiters, e.g. "/pattern/gi"`;
  }
  ...
  private async backendFileSet(args: Record<string, unknown>): Promise<ToolExecuteResult> {
    ...
    if (key === null) {
      return {
        content: `Error: invalid module path "${parsed.data.path}" — use slash-separated segments of [A-Za-z0-9_-] with a .lua extension (no "..", no leading "/")`,
      };
    }
    const character = await this.deps.characters.getById(parsed.data.characterId);
    if (!character) return { content: `Error: character "${parsed.data.characterId}" not found` };
